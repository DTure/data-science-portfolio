...
def get_sp500_tickers():
    ...
    # Yahoo Finance використовує "-" замість "." (наприклад, BRK-B → BRK.B)
    tickers = [ticker.replace('.', '-') for ticker in tickers]
    return tickers

tickers = get_sp500_tickers()
# 📅 Дата діапазон для короткострокового прогнозу (~1.5 роки)
...
def collect_data(tickers, start, end):
    ...
    for ticker in tickers:
        try:
            ...
            time.sleep(0.8)  # Невелика затримка
...
# 🧠 Додавання технічних індикаторів для короткострокового прогнозу
...
processed_data = raw_data.groupby("Ticker").apply(lambda x: add_short_term_indicators(x)).reset_index(drop=True)
processed_data.dropna(inplace=True)  # Видаляємо рядки з NaN

# 💾 Збереження у CSV
processed_data.to_csv("data/short_term.csv", index=False)

print("✅ Дані для короткострокового прогнозу збережені.")
